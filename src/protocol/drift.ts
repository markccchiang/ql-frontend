...
export function findDrift(reported: Capabilities): Drift[] {
    const drift: (Drift | null)[] = [
        ...
        // Unions across every trade, as for payoffs: which method a style
        // takes is a rule kept here, and the wire carries the set.
        compare("engine methods", offeredEngineMethods(), reported.engineMethods, asString),
        compare("finite-difference presets", FD_PRESETS, reported.fdPresets, asString),
        compare("result kinds", offeredResultKinds(), reported.resultKinds, asString),
