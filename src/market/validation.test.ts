...
const seed = () => [newQuote("S", 100, Quote_Unit.ABSOLUTE), newQuote("R", 0.05, Quote_Unit.RATE), flatCurve("RC", "R"), constantVol("VOL", "V"), newQuote("V", 0.2, Quote_Unit.VOLATILITY)];
...
describe("validateMarket", () => {
    ...
    it("a newly added curve carries no conventions, and says so", () => {
        ...
        expect(issues.map(issue => issue.path).sort()).toEqual(["yield_curve.day_counter", "yield_curve.flat.compounding", "yield_curve.flat.frequency"]);
    });
