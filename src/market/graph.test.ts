...
describe("dependenciesOf", () => {
    it("finds the quote a flat curve is built on", () => {
        expect(dependenciesOf(newFlatCurve("RC", "R"))).toEqual([{id: "R", path: "yield_curve.flat.rate.quote_id"}]);
    });

    it("finds the quote a constant vol is built on", () => {
        expect(dependenciesOf(newConstantVol("VOL", "V"))).toEqual([{id: "V", path: "volatility.constant.volatility.quote_id"}]);
    });
...
describe("topoSort", () => {
    it("puts dependencies before their dependents", () => {
        const {sorted, cycle} = topoSort([newFlatCurve("RC", "R"), newConstantVol("VOL", "V"), newQuote("R", 0.05), newQuote("V", 0.2)]);
