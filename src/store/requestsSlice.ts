...
export const selectInFlight = createSelector(
  [(state: WithRequests) => state.requests.order, (state: WithRequests) => state.requests.byId],
  (order, byId): RequestEntry[] =>
    order
      .map((id) => byId[id])
      .filter(
        (entry): entry is RequestEntry =>
