...
export const MarketObjectEditor = ({object}: {object: MarketObject}) => {
    ...
    const quoteOptions = quotes.filter(candidate => candidate.id !== object.id).map(candidate => ({value: candidate.id, label: candidate.displayName ? `${candidate.id} — ${candidate.displayName}` : candidate.id}));
    ...
    return (
        <Paper>
            <Group gap="xs" mb="xs" grow>
                ...
                />
                <TextInput size="xs" label="display name" value={object.displayName} onChange={event => dispatch(workbookActions.displayNameSet({id: object.id, displayName: event.currentTarget.value}))} />
            </Group>
        ...
        <div>
            <Group justify="space-between" mb={2}>
                ...
                <SegmentedControl
                    ...
                    onChange={value => onChange(value === "fixed" ? {case: "fixed", value: 0} : {case: "quoteId", value: ""})}
                />
            </Group>
            {mode === "quoteId" ? (
                <Select
                ...
                <NumberInput size="xs" decimalScale={6} error={error} value={source.case === "fixed" ? source.value : 0} onChange={value => onChange({case: "fixed", value: typeof value === "number" ? value : Number(value) || 0})} />
            )}
        </div>
    );
};
