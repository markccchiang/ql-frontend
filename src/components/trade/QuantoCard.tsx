...
export function QuantoCard() {
  ...
  const options = (predicate: (id: string) => boolean) =>
    market
      .filter((entry) => predicate(entry.id))
      .map((entry) => ({ value: entry.id, label: entry.displayName ? `${entry.id} — ${entry.displayName}` : entry.id }))
    ...
    <Paper>
      <Tooltip label={support.reason ?? 'Wraps the engine in a QuantoEngine and adjusts the dividend yield through a QuantoTermStructure.'} multiline w={280}>
        <Switch
          ...
          label="quanto"
          checked={on}
          disabled={support.availability !== 'supported'}
          onChange={(event) => dispatch(workbookActions.quantoToggled(event.currentTarget.checked))}
        />
      </Tooltip>
