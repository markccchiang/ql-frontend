...
export function UnderlyingCard() {
  ...
  const ids = (predicate: (id: string) => boolean) =>
    market.filter((object) => predicate(object.id)).map((object) => ({
      value: object.id,
  ...
  return (
    <Paper>
      ...
      <ChoiceSelect
        ...
        onChange={(next) => dispatch(workbookActions.processSet(next))}
      />
      ...
      {ref('dividend curve', 'dividendCurveId', curves, dividendIssue, {
        ...
        // the risk-free curve.
        description: noDividend
          ? 'Black-Scholes has no dividend yield'
          : underlying.dividendCurveId
            ? undefined
            : 'empty means a flat zero dividend yield',
      })}
