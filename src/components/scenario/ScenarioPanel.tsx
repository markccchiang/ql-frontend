...
export function ScenarioPanel() {
  ...
  const progress = useAppSelector((state) =>
    runningRequestId ? state.requests.byId[runningRequestId]?.progress ?? null : null,
  )
      ...
      <div style={{ width: 260, overflowY: 'auto', flexShrink: 0 }}>
        ...
        {spec.form === 'explicit' && (
          <TextInput
            ...
            mt={6}
            label="values"
            value={spec.explicit.join(', ')}
            onChange={(event) => dispatch(scenarioActions.specChanged({ explicit: numbers(event.currentTarget.value) }))}
          />
        )}
        ...
        <Tooltip label="A sweep is a question, not an edit. Leave this off and the quote is put back where it was." multiline w={260}>
          <Checkbox
            size="xs"
            mt={8}
            label="keep the last swept value"
            checked={spec.keepFinalValue}
            onChange={(event) => dispatch(scenarioActions.specChanged({ keepFinalValue: event.currentTarget.checked }))}
          />
        </Tooltip>
